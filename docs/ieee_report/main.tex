...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{figures/pretrain_loss.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{figures/v5_regression.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{figures/sims_scaling.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{figures/v7_progress.pdf}
...
difference from its starting point, both models beating a random mover
...
The guarantee is weaker than it appears: 8 of 20 gates promoted against
7.8 expected under the null (Section~\ref{sec:discussion}).
...
The value head plateaus at MSE 0.73, and absolute strength is uncalibrated
against a rated engine, so all results are relative.
...
\begin{itemize}[noitemsep]
  \item \textbf{Code:} \url{https://github.com/realgauravvyas/chess-ai}
        --- network, search, both training stages, the dashboard, and the
        scripts reproducing every number in this report.
        \texttt{RESULTS.md} records the findings and the defects fixed.
  \item \textbf{Live demo:} \url{https://realgauravvyas.github.io/chess-ai/} --- play the trained
        network in a browser, with the search's principal variation, its
        candidate moves and the training curves.
  \item \textbf{Video:} \url{https://youtu.be/MQH5u9tvdEM}
\end{itemize}

\noindent
\noindent\textbf{Disclaimer.} AI-assisted tools were used during the
development of this project, for code review, debugging and drafting. All
experimental results reported here were produced by the author's own runs
and are reproducible from the linked repository.
